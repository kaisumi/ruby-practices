...
class Game
  ...
  def score
    shots_to_frames(@text_scores)
  end

  private

  def shots_to_frames(text_scores)
    shot_idx = 0
    frames = []
    shots = text_scores.map do |score|
      Shot.new(score)
    end
    ...
    NUMBER_OF_FRAMES.times do # フレームごとにスコアを加算。ストライク：3ショット、スペア：3ショット、その他：2ショット
      ...
      if frame.strike_or_spare?
        raise NOT_ENOUGH_ERROR if shots[shot_idx + TO_THIRD].nil?

        frame << shots[shot_idx + TO_THIRD] # 3ショット目（ストライクまたはスペアの場合）
      end
